...
# A class to handle realsense camera for ros
class RealsenseCamera(object):
    def __init__(self, topic_color, topic_depth) -> None:
        self.topic_color = topic_color
        self.topic_depth = topic_depth
        self.bridge = CvBridge()
        self.color = None
        self.depth = None
        print("Calling color image subscriber")
        self.color_sub = rospy.Subscriber(self.topic_color, Image, self.color_callback)
        self.depth_sub = rospy.Subscriber(self.topic_depth, Image, self.depth_callback)
        print("Called both subscribers")
...
class V4L2Camera(object):
    def __init__(self, index=4):
        ...
        print(self._cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        print(self._cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
...
if __name__ == '__main__':
    ...
    # # Loop through possible device indices
    # for i in range(10):
    #     # Try to open the device
    #     cap = cv2.VideoCapture(i)
    #     # Check if it is opened
    #     if cap.isOpened():
    #         # Print the index of the camera
    #         print(f"Camera {i} is available.")
    #         # Release the device
    #         cap.release()

    # cam = V4L2Camera(index=2)
    cam = RealsenseCamera(topic_color='/camera/color/image_raw', topic_depth='/camera/depth/image_rect_raw')
    cnts = 0
